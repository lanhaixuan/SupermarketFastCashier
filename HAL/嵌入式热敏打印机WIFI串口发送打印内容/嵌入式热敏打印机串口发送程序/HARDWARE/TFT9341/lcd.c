...
{ 					
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_FSMC,ENABLE);	//使能FSMC时钟
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOG,ENABLE);//使能PORTB,D,E,G以及AFIO复用功能时钟
...
//size:字体大小
//lenZ:整数部分的长度
//lenX:小数部分的长度 最长为4位小数是准确的 超过4位不准确
//num:数值
...
void LCD_ShowCh(u16 x,u16 y,u8 size,u8 num)
{  							  
  u8 temp,t1,t,size_all;
	u16 y0=y;
  ...
  num=num*2;
	if(size==16)
	  ...
	  for(t=0;t<size_all;t++)
	  {   
			if(size==32)temp=chinese_3216[num][t];  //??1206??
			else if(size==16) temp=chinese_1608[num][t];		 //??1608?? 	                          
	    for(t1=0;t1<8;t1++)
			{			    
...
void LCD_ShowChinese(u8 size,u16 x,u16 y,u16 width,u16 height,u8 start,u8 end)
{         
	u8 x0=x,i;
	width+=x;
	height+=y;
